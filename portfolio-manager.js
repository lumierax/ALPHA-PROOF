'use strict';
const TradingDay=require('./execution/trading-day.js');
...
function configuredMode(cfg={}){
  ...
  return String(cfg.mode??process.env.PORTFOLIO_MODE??'PAPER').toUpperCase()==='PAPER'?'PAPER':'BINANCE';
}
function policy(cfg={}){
...
function normalize(raw,cfg={}){
  ...
  const numericStateInvalid=['cashUSDT','initialCapitalUSDT','realizedPnlUSDT','feesUSDT'].some(k=>x[k]!==undefined&&(typeof x[k]!=='number'||!Number.isFinite(x[k])));
  const invalidPositions=Object.entries(x.positions||{}).some(([id,p])=>!normalizePosition(p,id));
  const out={
    ...base,...x,
    integrityBlocked:!!x.integrityBlocked||numericStateInvalid||invalidPositions,
    schema:SCHEMA,
    scope:SCOPE,
    mode,
    exchange:EXCHANGE,
    ...
    cashUSDT:finite(x.cashUSDT,base.cashUSDT),
...
function open(raw,trade,now=Date.now(),opts={}){
  ...
  if(!integrity(raw,opts.marks||{}).ok)return reject(s,'PORTFOLIO_INTEGRITY_BLOCK',{},now);
...
function close(raw,id,price,now=Date.now(),opts={}){
  ...
  if(!p)return s;
  if(!integrity(raw,opts.marks||{}).ok)return reject(s,'PORTFOLIO_INTEGRITY_BLOCK',{id},now);
...
function integrity(raw,marks={}){
  ...
  const rawCoreValid=['cashUSDT','initialCapitalUSDT','realizedPnlUSDT','feesUSDT'].every(k=>raw?.[k]===undefined||(typeof raw[k]==='number'&&Number.isFinite(raw[k])));
  const finiteCore=!s.integrityBlocked&&rawCoreValid&&[s.initialCapitalUSDT,s.cashUSDT,s.realizedPnlUSDT,s.feesUSDT,v.liquidationEquityUSDT,v.totalPnlUSDT].every(Number.isFinite);
...
function publicState(raw,marks={}){
  ...
  return {
    ...
    accounting:{basis:'NET_LIQUIDATION_AFTER_ESTIMATED_EXIT_FEE',feePolicy:FEE_POLICY,entryFeePct:ENTRY_FEE_PCT,exitFeePct:EXIT_FEE_PCT,roundTripFeePct:ROUND_TRIP_FEE_PCT,timezone:TradingDay.TIMEZONE,dayBoundary:TradingDay.BASIS},
