...
for(const file of ['index.html','public/index.html']){
 ...
 assert(s.includes('10.0.0-FULL-PROFIT-FIRST-RESEARCH-ARCH-CAPTURE-1'),'v10 release version missing');
 ...
 assert(s.includes('function renderCortexWorkforce()')&&s.includes('renderCortex();renderCortexWorkforce();renderV10Research();'),'workforce/v10 renderers not wired into AI page');
 ...
 // v10 research architecture: all 14 built, prospective/shadow governed, visible and recoverable.
 for(const id of ['v10ResearchPanel','v10Authority','v10Opportunities','v10Labels','v10ManagementTrials','v10Regime','v10ReplayUpdates','v10IndependentEvidence','v10DepthSnapshots','v10TeacherSamples','v10PolicyBody','v10Capabilities','v10RegimeProbs','v10Teacher','recV10Research'])assert(s.includes(`id="${id}"`),`v10 UI missing ${id}`);
 assert(s.includes('ALPHA PROOF v10 — Research Architecture')&&s.includes('القدرات البحثية الـ14'),'v10 research architecture wording missing');
 assert(s.includes('function renderV10Research()')&&s.includes('NO_LIVE_AUTHORITY'),'v10 shadow governance renderer missing');
 assert(s.includes("['v10Research','recV10Research']"),'v10 recovery component not shown');
 assert(s.includes("['v10 Research',parts.v10Research"),'v10 memory breakdown missing');
...
assert(b.includes('const V10Research=(()=>{')&&b.includes('async function localV10Load()')&&b.includes('async function localV10Persist('),'GitHub backup must embed/persist v10 research engine');
assert(b.includes("indexedDB.open(LOCAL_MEMORY_DB,3)")&&b.includes("createObjectStore('v10Brain'"),'local v10 A/B store missing');
assert(b.includes('await localV10Register(w,r.shadow,now)')&&b.includes('await localV10Advance(tickerMap,contexts,now)'),'local v10 prospective hooks missing');
