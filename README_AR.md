# ALPHA PROOF Ω — v9.8.1 FREE MANAGEMENT · KLINE TRUTH · LAB AI Ω CORTEX

## v9.8.1 — Railway Memory Safe
- إصلاح انهيار Node `heap out of memory` بدون حذف Dataset أو AI Brain أو Market AI أو CORTEX أو Checkpoints.
- عند استعادة state كبير، لا يعاد تحميل نسخة `liveRows` المكررة؛ يعاد بناؤها من `active`.
- حفظ state لا يكرر `liveRows` داخل الملف.
- Adaptive Manager يحتفظ في RAM فقط بالنافذة اللازمة للتعلم والقرارات الحديثة، مع بقاء حالة الصفقة الأساسية مستمرة. هذا حد ذاكرة تقني وليس شرط تداول أو مدة قصوى للصفقة.
- Integrity Replay يحتفظ في RAM ببيانات replay الضرورية فقط بدل الاحتفاظ بالـjourney التاريخي الكامل أثناء إعادة البناء. المصدر الكامل يبقى على القرص.
- أضيف اختبار ضغط Railway بحد Node heap مقداره 96 MB مع state قديم يتجاوز 20 MB.
...
اعتبر **v9.7.0 MULTI-TF AUDIT · LAB AI Ω CORTEX** خط الأساس التالي بعد نجاح اختبارات الحزمة النهائية.
...
## v9.7.0 — تدقيق سعر AI + جدولة مستقلة لكل فريم

- جدول **آخر قرارات وتجارب AI** يعرض الآن سعر العملة الذي استخدمه Trade AI عند الدخول (`fillPrice`) وسعر الخروج الذي استُخدم فعليًا في تسوية Shadow (`outcome.exitPrice`) بجانب نتيجة الربح/الخسارة الصافية. الهدف هو تمكين التدقيق اليدوي وعدم الاكتفاء بتصنيف WIN/LOSS.
- الفحص التلقائي لم يعد دورة واحدة على فريم واحد. عند اختيار وضع **تلقائي** يفحص جميع أزواج Spot/USDT المؤهلة على جدول مستقل: `5m` كل 5 دقائق، `15m` كل 15 دقيقة، `30m` كل 30 دقيقة، `1h` كل ساعة، `4h` كل 4 ساعات، `6h` كل 6 ساعات، `12h` كل 12 ساعة، و`1d` يوميًا.
- كل موعد مرتبط بحدود شموع Binance ويبدأ بعد الإغلاق بدقيقة. لذلك فحص `1d` يكون عند **03:01 بتوقيت مكة (Asia/Riyadh)**، مع أوقات 4h/6h/12h الموافقة لحدود Binance نفسها.
- إذا استحقت عدة فريمات في الدقيقة نفسها، تدخل في طابور واحد وتُفحص تباعًا حتى لا تتزاحم طلبات Binance ولا يُشغَّل أكثر من فحص سوق كامل في اللحظة نفسها.
- الفحص المجدول يحلل **كامل Universe المؤهل** في كل فريم، بما في ذلك العملات الموجودة أصلًا في الرادار؛ Lux يبقى بوابة دخول الرادار الوحيدة، وDirectional Flow يبقى مسؤول ترتيب/اختيار الفريم الأفضل.
- اختيار الفريم أعلى الواجهة بقي للفحص اليدوي فقط؛ الجدولة التلقائية لا تعتمد عليه.


## v9.8 — Binance Kline Truth + Adaptive Manager
- الساعة المحلية ليست مصدر حقيقة لإغلاق الشمعة. المؤقت يوقظ المحرك قرب الحد المتوقع فقط، ثم يؤكد آخر Kline مغلق من Binance قبل الفحص الكامل.
- لكل فريم `lastProcessedClosedCandle` لمنع التكرار واستدراك الإغلاق بعد إعادة التشغيل. إذا لم تؤكد Binance الإغلاق بعد، يعاد التحقق بطلب خفيف دون تشغيل Full Market Scan.
- أضيف Adaptive Manager كـ Shadow Challenger مستقل: لا يستخدم TP أو SL ثابتين، ويتعلم HOLD/EXIT من ملاحظات مجمدة وقت القرار ثم تسميات لا تنضج إلا بعد بيانات لاحقة.
- Baseline القديم يبقى للمقارنة ولا يتحكم في Adaptive Manager.
- رسوم Spot للقياس الاقتصادي ثابتة 0.20% بدون خصم BNB.
- لا توجد أوامر تداول حقيقية.
