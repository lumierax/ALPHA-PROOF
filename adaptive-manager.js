...
// Adaptive management learner. It manages decision economics independently of venue;
// actual placement remains delegated to the execution engines.
...
function positionNet(pos,price){let raw=gross(pos.dir,pos.entryPrice,price),profile=pos.executionProfile||{venue:'SPOT',leverage:1},isFutures=profile.venue==='FUTURES',lev=isFutures?Math.max(1,Number(profile.leverage)||1):1,fee=isFutures?2*Math.max(0,Number(profile.commissionPct)||.05)*lev:.20,entryCost=Math.max(0,Number(pos.entryExecutionCostPct)||0)*(isFutures?lev:1);return raw*lev-fee-entryCost}
function features(pos,w,price,now){let net=positionNet(pos,price),peak=Math.max(Number(pos.peakNetPct??net),net),f=w?.flow||{};return {net:clamp(net/5,-1,1),giveback:clamp((peak-net)/5,0,1),flowRelative:clamp((Number(f.relative)||0)/5,0,1),pressure:clamp(((Number(f.pressure)||50)-50)/50,-1,1),persistence:clamp((Number(f.persistence)||0)/100,0,1),speed:clamp((Number(f.speed)||0)/2,-1,1),acceleration:clamp((Number(f.acceleration)||0)/.5,-1,1),radar:clamp((Number(w?.radarScore)||0)/100,0,1),age:clamp((now-pos.entryAt)/(24*3600000),0,1),entryConfidence:clamp(Number(pos.entryConfidence)||0,0,1)}}
...
function captureState(s,now){
 if(!s.capture)s.capture={policy:CAPTURE_POLICY,startedAt:now,entryCells:{},holdCells:{},studies:{},completed:[],samples:0,unresolved:0,capacitySkips:0};
...
function captureNet(pos,w,price,now){return gross('LONG',pos.entryPrice,price)-.20-pos.entryCostPct-quoteCost(w,price,'SELL',now)*(price/pos.entryPrice)}
...
function capturePublic(state,now=Date.now()){
 ...
 return {policy:c.policy,status:'PROSPECTIVE_SHADOW',startedAt:c.startedAt,samples:c.samples,studies:Object.keys(c.studies).length,unresolved:c.unresolved,capacitySkips:c.capacitySkips,entryEvidence:entries,completed:c.completed.slice(-40).reverse(),costPolicy:'OBSERVED_SPREAD_PLUS_IMPACT_SCENARIO',comparisonScope:'PAIRED_PER_TRADE_NOT_PORTFOLIO_ROI'};
}

module.exports={positionNet,validateCaptureState,CAPTURE_POLICY,entryAdvice,advanceCapture,capturePublic,frameMinutes,SCHEMA,MODEL_VERSION,FEATURE_SCHEMA,FEATURES,OBS_RAM_MAX,DECISION_RAM_MAX,CLOSED_RAM_MAX,RECENT_RAM_MAX,initialState,normalizeState,open,observe,noteBaseline,compactForPersistence,publicState};
