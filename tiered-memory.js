...
function createTieredMemory({dataDir,stateFile,aiFoundation,marketAiFoundation,learningForgeFoundation,adaptiveFoundation,v10ResearchFoundation,coldRuntime}){
 const totals={capturedAt:Date.now(),ramLimitBytes:cgroupLimit(),volumeTotalBytes:volumeTotal(dataDir)};
 function breakdown(){
  let mu=process.memoryUsage(),stateBytes=fileBytes(stateFile),labMemory=dirBytes(path.join(dataDir,'lab-memory')),ai=dirBytes(aiFoundation?.root||path.join(dataDir,'ai-foundation')),market=dirBytes(path.join(dataDir,'market-ai-foundation')),cortex=dirBytes(learningForgeFoundation?.root||path.join(dataDir,'omega-cortex')),adaptive=dirBytes(adaptiveFoundation?.root||path.join(dataDir,'adaptive-manager')),v10Research=dirBytes(v10ResearchFoundation?.root||path.join(dataDir,'v10-research')),coldRuntimeBytes=dirBytes(path.join(dataDir,'cold-runtime')),integrity=['integrity-raw-backup-v9.5','integrity-replay-net-pnl.json','integrity-sanitize-net-pnl.json'].reduce((a,n)=>a+dirBytes(path.join(dataDir,n)),0),known=stateBytes+labMemory+ai+market+cortex+adaptive+v10Research+coldRuntimeBytes+integrity,totalUsed=dirBytes(dataDir),other=Math.max(0,totalUsed-known);
  return {measuredAt:Date.now(),totals,ram:{rssBytes:mu.rss,heapUsedBytes:mu.heapUsed,heapTotalBytes:mu.heapTotal,externalBytes:mu.external,arrayBuffersBytes:mu.arrayBuffers||0,remainingToLimitBytes:totals.ramLimitBytes?Math.max(0,totals.ramLimitBytes-mu.rss):null},storage:{usedBytes:totalUsed,remainingBytes:totals.volumeTotalBytes!=null?Math.max(0,totals.volumeTotalBytes-totalUsed):null,parts:{runtimeState:stateBytes,labDataset:labMemory,tradeAI:ai,marketAI:market,cortex,adaptiveManager:adaptive,v10Research,coldRuntime:coldRuntimeBytes,integrityAudit:integrity,other}}};
 }
