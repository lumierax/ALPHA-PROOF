...
function createTieredMemory({dataDir,stateFile,aiFoundation,marketAiFoundation,learningForgeFoundation,adaptiveFoundation,coldRuntime}){
 ...
 function breakdown(){
  let mu=process.memoryUsage(),stateBytes=fileBytes(stateFile),labMemory=dirBytes(path.join(dataDir,'lab-memory')),ai=dirBytes(path.join(dataDir,'ai-foundation')),market=dirBytes(path.join(dataDir,'market-ai-foundation')),cortex=dirBytes(path.join(dataDir,'omega-cortex')),adaptive=dirBytes(path.join(dataDir,'adaptive-manager')),coldRuntimeBytes=dirBytes(path.join(dataDir,'cold-runtime')),integrity=['integrity-raw-backup-v9.5','integrity-replay-net-pnl.json','integrity-sanitize-net-pnl.json'].reduce((a,n)=>a+dirBytes(path.join(dataDir,n)),0),known=stateBytes+labMemory+ai+market+cortex+adaptive+coldRuntimeBytes+integrity,totalUsed=dirBytes(dataDir),other=Math.max(0,totalUsed-known);
