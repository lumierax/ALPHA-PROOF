...
const SPOT_ENTRY_FEE_PCT=.10,SPOT_EXIT_FEE_PCT=.10,SPOT_ROUND_TRIP_FEE_PCT=SPOT_ENTRY_FEE_PCT+SPOT_EXIT_FEE_PCT;
const SPOT_ENTRY_FEE_RATE=SPOT_ENTRY_FEE_PCT/100,SPOT_EXIT_FEE_RATE=SPOT_EXIT_FEE_PCT/100;
const FEE_TOLERANCE=1e-9;
const ORIGINAL_CUTOFF=Date.parse('2026-09-06T11:50:00+03:00');
const OBJECTIVE=Object.freeze({id:'REPEATABLE_NET_ECONOMIC_EDGE',statement:'اكتشاف أفضلية اقتصادية مستقبلية قابلة للتكرار بعد جميع التكاليف، ومعرفة متى توجد ومتى لا توجد.',feePolicy:'SPOT_NO_BNB',roundTripFeePct:SPOT_ROUND_TRIP_FEE_PCT,proof:'FROZEN_THEN_PROSPECTIVE',calibrationIsProfit:false,realExecution:false});
...
function validOutcome(shadow){const o=shadow?.outcome;return ['ENTER','SKIP'].includes(shadow?.finalDecision)&&o?.executionValid===true&&Number.isFinite(o.netPnlPct)&&Number.isFinite(o.grossMovePct)&&o.feePolicy==='SPOT_NO_BNB'&&Math.abs(o.totalFeesPct-SPOT_ROUND_TRIP_FEE_PCT)<=FEE_TOLERANCE&&(o.executionPolicy!=='SPOT-CAPTURE-1'||Number.isFinite(o.executionCostPct)&&o.executionCostPct>=0)&&Math.abs(o.netPnlPct-(o.grossMovePct-SPOT_ROUND_TRIP_FEE_PCT-(o.executionPolicy==='SPOT-CAPTURE-1'?o.executionCostPct:0)))<.00003&&o.winLabel===(o.netPnlPct>0?1:0);}
module.exports={validOutcome,POLICY,ORIGINAL_CUTOFF,OBJECTIVE,SPOT_ENTRY_FEE_PCT,SPOT_EXIT_FEE_PCT,SPOT_ROUND_TRIP_FEE_PCT,SPOT_ENTRY_FEE_RATE,SPOT_EXIT_FEE_RATE,FEE_TOLERANCE,createCohort,eligible};
